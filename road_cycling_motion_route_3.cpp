...
// g++ road_cycling_motion_route_3.cpp -o t -I/usr/include/python3.8 -lpython3.8
...
{
    auto dt_1 = (double)(1200.0 / nr_iter);
    std::vector<R> route;
    std::vector<R> velo;
    double L = 1000.0;

    for (auto ii = 0.0; ii <= 1200.0; ii = ii + dt_1)
    {

        R r_i;
        r_i.x = ii;
        r_i.y = 125 * std::sin(5 * M_PI * ii / L);
        r_i.z = (L - ii) / 20.0;
        route.push_back(r_i);
    }
...
double fg(double u)
{
    ...
    return -m * g * std::sin(45.0 / 1000.0);
}
...
void plot3D(std::vector<double> xX, std::vector<double> yY, std::vector<double> zZ)
{
